...
contract PlayGame {
    ...
    uint [] price;
    // 返回是否下注成功
    event returnBetResult(bool _bool, address _addr, uint betTime);
    event emitXor(uint xor, uint dNum, uint tNum);
    // 返回最终结果
    ...
    constructor(uint price1, uint price2, uint price3, uint price4, string _name)public{
    ...
    function getBlockTime() public constant returns (uint, uint, uint, address, uint [], int){
        return (time, block.timestamp, xor, creator, currentResult, uNumTest);
    }
    // 获取最后五局的结果
    function getResultHistory() public constant returns (uint []){
    ...
    modifier betLimit(uint _value, uint _cho) {
        ...
        if (_cho == 0 || _cho == 1) {// 龙
            require(thisTotal * 2 <= balance, "下注已经封盘");
        } else if (_cho == 2) {// 合
            require(thisTotal * 9 <= balance, "下注已经封盘");
        }
        _;
    }
    // 下注传入信息
    // 如果下注金额大于当前奖池金额，返回false，下注失败
    function sendBetInfo(address addr, uint cho, uint ran, uint coin) betLimit(msg.value, cho) public payable {
        totalCoins += coin;
        deposit();
        ...
        emit returnBetResult(true, addr, latestBetTime);
    }
    ...
    function xorFun() private {
        ...
        xor = xor ^ (rom % 1000000000000);
    }

    // 获取当前出块时间戳 (单位是秒)
    function getTimestamp() private constant returns (uint) {
    ...
    function drawings(uint _coin) public payable {
        if (msg.sender == creator) {
            ...
            if (_balance >= totalCoins * 9 + _coin) {
    ...
    function getResult() public {
        ...
        if (_times - time > 10 && latestBetTime != defaultTimt) {
            uNumTest++;
            //异或后得到了12位的随机数
            xorFun();
            processXor();
    ...
    // 返回下注金额
    function getTotalCoins() public constant returns (uint, uint, uint, uint) {
