...
contract Quiz {
  ...
  event returnBetResult(uint code, string msg, uint hCore, uint vCore); // 返回是否下注成功 101-"已过下注截止时间" 102-"比赛未结束不能结算" 201-"下注成功" 205-"结算完成"
  ...
  */
  constructor(string _name, string _homeTeam, string _visitingTeam, uint _oddsH, uint _oddsD, uint _oddsV, uint _deadline, uint _singleCoin, uint _hConcedePoints, uint _vConcedePoints, string _liveId) public{
    contractName = _name;
    homeTeam = _homeTeam;
  ...
  function getLiveId()public view returns(string){
    return liveId;
  }
  ...
  // 返回配置参数
  function getSetting() public constant returns ( string, string, string, string, uint[7]) {
    uint [7] memory info;
    info = [oddsH, oddsD, oddsV, deadline, singleBetCoin, hConcedePoints, vConcedePoints];
    return ( contractName, homeTeam, visitingTeam, liveId, info);
  }
  ...
  */
  ...
  function drawings(uint _coin) public payable {
    if (msg.sender == creator && ((block.timestamp + 86400) * 1000 ) > deadline && isDone) {
      uint _balance = getCurrentBalance();
      transferCoin(creator, _balance);
    } else {
      emit returnBetResult(102, "比赛未结束不能结算", 0, 0);
    }
  }
